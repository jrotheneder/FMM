...
std::array<vector<double>, 8> par_fmm_efficiencies(int N, double eps, size_t s, 
    ...
    // in retrospect vector<vector<double>> might have been better lol
    vector<double> nprocs, tree_timings, pot_eval_timings, 
        frc_eval_timings, total_fmm_timings, dir_pot_timings, 
        dir_frc_timings, total_dir_timings; 
    ...
    for(int i = min_procs; i <= max_procs; i+=incr) {
        ...
        auto ref_potentials = particlePotentialEnergies<d, field_type>(sources);
...
//      auto accuracies = abs_rel_frc_error(forces, ref_forces);  
        ...
        total_fmm_timings.push_back(t4-t1);
        dir_pot_timings.push_back(t5-t4);
        dir_frc_timings.push_back(t6-t5);
        total_dir_timings.push_back(t6-t4);
    ...
    return {nprocs, tree_timings, pot_eval_timings, frc_eval_timings, 
        total_fmm_timings, dir_pot_timings, dir_frc_timings, total_dir_timings}; 
}

void logEfficiencesToFile(array<vector<double>, 8> efficiencies, string head, 
        string filename) {

    auto [nprocs, tree_timings, pot_eval_timings, frc_eval_timings, total_fmm_timings,
    dir_pot_timings, dir_frc_timings, total_dir_timings] = efficiencies;
    ...
    printIterable(total_fmm_timings);
    printIterable(dir_pot_timings);
    printIterable(dir_frc_timings);
    printIterable(total_dir_timings);
}
...
//
    ...
    for(bool uniform_sources : {true, false}) {
        for(bool uniform_tree : {true, false}) {

        if(!uniform_sources && uniform_tree) { continue; } // basically direct algo

        string head = "nprocs\tTree\tPot. (FMM)\tForce (FMM)\tTotal (FMM)"
            "\tPot. (direct)\tForce (direct)\tTotal (direct)\n";
